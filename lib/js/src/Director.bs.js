...
function parse(param) {
  ...
  var name = param.name;
  return Result$WonderEngineAbstract.bind(_findGroup(param.firstGroup, groups), (function (group) {
...
function init(param) {
  ...
  return parse(Curry._1(DpContainer$WonderEngineAbstract.unsafeGetSceneRenderWorkDp(undefined).getInitPipelineData, undefined));
}

function update(param) {
  return parse(Curry._1(DpContainer$WonderEngineAbstract.unsafeGetSceneRenderWorkDp(undefined).getUpdatePipelineData, undefined));
}

function render(param) {
  return parse(Curry._1(DpContainer$WonderEngineAbstract.unsafeGetSceneRenderWorkDp(undefined).getRenderPipelineData, undefined));
}
