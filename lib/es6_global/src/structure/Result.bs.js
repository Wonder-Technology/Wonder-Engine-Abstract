...
import * as Exception$WonderEngineAbstract from "./Exception.bs.js";
...
var _buildErr = Exception$WonderEngineAbstract.buildErr;

function failWith(x) {
  return {
          TAG: /* Error */1,
          _0: Exception$WonderEngineAbstract.buildErr(x)
        };
}
...
function tryCatch(oneTrackFunc) {
  try {
    ...
    if (e.RE_EXN_ID === Js_exn.$$Error) {
      ...
      return {
              TAG: /* Error */1,
              _0: Exception$WonderEngineAbstract.buildErr("unknown error: " + e)
            };
    }
  }
}
...
function getExn(result) {
  return handleFail(result, Exception$WonderEngineAbstract.throwErr);
}
